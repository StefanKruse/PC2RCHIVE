...
## debugging script: observe(print(str(text()))) - print out structure of variable
...
ui <- fluidPage(theme = shinytheme("united"), ## for different theme: https://shiny.posit.co/r/getstarted/build-an-app/customizing-ui/theming.html
                #### theme, color ####
                ...
                tabPanel("General information", value = "tabPanel1",# tabPanel1
                         mainPanel(
                           uiOutput("htmlfile"), # works with that too (in that case no renderUI is necessary!): includeMarkdown(html) Source:https://stackoverflow.com/questions/75754314/how-to-render-the-contents-of-a-word-document-including-formats-in-a-shiny-app-i
                ...
                tabPanel("Reading point cloud data", value = "tabPanel2", # tabPanel2,
                         wellPanel(
                           ...
                           conditionalPanel(condition = "input.outputfolder_selection == 'v2_output'",
                                            textInput("outputfolder_typed", h5("Path of directory"), ""),
                         ...
                         wellPanel(
                           ...
                           conditionalPanel(condition = "input.trajectory_available == 1",
                                            ...
                                            #fileInput("input_trajectory", "Select data"),
                         ...
                         tabsetPanel(id="data_processing",
                                     tabPanel("Ground classification", value = "tabPanel3_1",
                                              ...
                                              checkboxGroupInput("classification_selection", "Select one:",
                                                                 #selectInput("classification_selection", "Select one:",
                                              ...
                                              conditionalPanel(condition = "input.classification_selection == 'v2'",
                                                               wellPanel(
                                                                 ...
                                                                 h5("Point clouds are generated from ground and above-ground points."),
                                                                 h5("Note: if the point cloud is too big (LAZ file is ~1,5 GB) visualization can fail. In that case skip visualization and directly save the data."),
                                                                 ...
                                                                 #textInput("cl_file_name", h4("File name"), "ground_classification"), # user created file names???
                         ...
                         tabsetPanel(id = "indiv_tree_segm",
                           ...
                           tabPanel("Creating point cloud of trees", value = "tabPanel4_2",
                                    sidebarPanel(
                                      ...
                                      #br(),
                                      #h5("Load point cloud with ground classification, if the file is not loaded in the program (created during Ground classification)"),
                                      #h5("Do not tick the box if the statement is not true!"),
                                      #checkboxInput("reload_PC_cl", "Reload point cloud with ground classification"),
                                      #conditionalPanel(condition = "input.reload_PC_cl == 1",
                                      #                 #actionButton("reload_PC_cl_button", "Load file", class = "btn-primary")
                                      #                 ),
                                      #br(),
                                      #actionButton("add_treeID_PC", "Add tree ID-s to the loaded point cloud", class = "btn-primary"),
                                      br(),
                                      br(),
                           ...
                           tabPanel("Tree statistics", value = "tabPanel4_3",
                                    sidebarPanel(
                                      ...
                                      #numericInput("minHeight_forStatistics", "Minimum tree height", 0.4),
                                      #numericInput("number_of_breaks", "Number of bins", 20),
                ...
                tabPanel("User provided metadata", value = "tabPanel6", # tabPanel6
                         ...
                         tabPanel("Metadata", value = "tabPanel6_2",
                           ...
                           #dateInput("dateOfRecording", "Date of data collection", ""),
                           ...
                           #h4("Table"),
                           #h5(strong("Table is generated from user input and relevant parameters/results of the data processing progress")),
...
server <- function(input, output, session){
  ...
  output$outputfolder_dir_typed <- #eventReactive(input$input$select_typed_folder, {
    renderText({
      #if(input$select_typed_folder>0){
  ...
  ## filter for trajectory where data was recorded (based on point cloud GPS times)
  ...
  traj_hull <- eventReactive(input$create_traj_hull, {
    ...
    concaveman::concaveman(traj_coord_sf(), concavity = input$concavity, length_threshold = 0)
    }) 
  ...
  output$crownPoly_treeTops <- renderPlot({
    ...
    #if(input$submit_treePoly_map>0) {
  ...
  #ground_cl <- eventReactive(input$reload_PC_cl_button, {
  ground_cl <- reactive({
  ...
  #ground_cl_withID <- eventReactive(input$add_treeID_PC , {
  ...
  osm <- reactive({ 
    ...
    read_osm(map_boundary()) ## https://stackoverflow.com/questions/76870373/how-to-crop-a-small-area-from-leaflet-or-tmap
  })
  ...
  output$downloadData_esri <- downloadHandler(
    ...
    content = function(file) {
      ...
      id <- showNotification("Downloading image...", duration = NULL, closeButton = FALSE) # create notification for user: https://mastering-shiny.org/action-feedback.html#transient-notification
      on.exit(removeNotification(id), add = TRUE)
      
      mapview::mapshot(x = user_created_map1(), file = file
                       , selfcontained = FALSE) 
      #errors -> required: library(webshot) + run webshot::install_phantomjs() # https://stackoverflow.com/questions/59519438/download-leaflet-map-as-a-png-when-shiny-app-is-deployed
    } # content()
  ) # downloadHandler()
  ...
  df_contributors <- reactiveValues(loaded_csv = NULL) # https://forum.posit.co/t/shiny-dt-once-you-edit-can-you-save-to-the-file-you-opened-from/90304/3
  # https://forum.posit.co/t/editable-dt-table-with-content-coming-from-uploaded-file/91950/3
  observe({
    ## check availability
  ...
  ## Load master table
  ## Create reactive value in which loaded metadata table will be fed
  vals <- reactiveValues(loaded_csv = NULL) # https://forum.posit.co/t/shiny-dt-once-you-edit-can-you-save-to-the-file-you-opened-from/90304/3
  # https://forum.posit.co/t/editable-dt-table-with-content-coming-from-uploaded-file/91950/3
  observe({
    ## check availability
